...
public class ContratoReport {
	...
	public byte[] generate(ContratoTO c) {
	...
	private Table buildHeader(DocumentBuilder db) {
		...
		table.addCell(db.createParagraphCell(12, contrato.getNomeEmissora(), new Style().setTextAlignment(TextAlignment.CENTER)));
	...
	private Table buildTermsTable(DocumentBuilder db) {
		...
		String terms = String.format(initTerms, contrato.getNomeEmissora(), contrato.getLogradouroEmissora(), contrato.getNumeroEmissora(), contrato.getBairroEmissora(), contrato.getCidadeEmissora(), contrato.getEstadoEmissora(), contrato.getCnpjCliente());
	...
	private Table buildInfoTable(DocumentBuilder db) {
        ...
        Style titleStyle = new Style().setFont(baseFontBold).setFontSize(12).setTextAlignment(TextAlignment.CENTER);
        ...
        infoTable.addCell(db.createLabelCell(3, "Início: ", contrato.getInicio()));
